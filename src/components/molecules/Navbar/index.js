...
export default function Navbar() {
  ...
  return (
    <div
    className={`visible ${show && "nav-hidden"} shadow-lg`}
    style={{ backgroundColor: "#31BF73" }}
    >
      <div className="flex items-center justify-between w-full px-3 py-3 md:px-24">
        <div>
          <Link to="/">
            <img
              className="max-w-10 max-h-12"
              src={logo}
            ...
            {navLinks.map((navItem) => (
              <li className="mx-4" key={navItem.title}>
                <NavLink
                  to={navItem.link}
                  style={activeLink}
                  className="duration-300 text-accent hover:text-black"
                >
                  {navItem.title}
                </NavLink>
              </li>
